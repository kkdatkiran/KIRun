...
<tr><td style="padding:10px">
	...
	<tr><td style="background-color: #FFFFFF;padding-top:20px" align="center">
		<h1 style="font-size: 25px; line-height: 35px; font-weight: bold; color: #064a5a">Verify email address and activate user for KIRun</h1>
		<div style="width: 100%; font-size: 15px; color: #000; padding: 30px 45px; box-sizing: border-box; min-height: 300px; text-align: justify">
			...
			<a href="${urlPrefix}/api/security/user/activateUser?userId=${user.email}&activationCode=${user.activationString}" style="text-decoration:none;line-height:100%;background:#53a8bc;color:white;font-family:Ubuntu,Helvetica,Arial,sans-serif;font-size:15px;font-weight:normal;text-transform:none;margin:0px;padding: 10px 30px;border-radius:3px">Verify</a>
			</div>
			If the above button don't work please copy the link below and open in a browser.
			<span style="color: #53a8bc;font-weight:bold; display:block;padding:10px;text-align:center">${urlPrefix}/api/security/user/activateUser?userId=${user.email}&activationCode=${user.activationString}</span>	
		</div>
	</td></tr>
