...
public class RuntimeAuthenticationFailureHandler implements AuthenticationFailureHandler {
	...
	private static final Logger logger = LoggerFactory.getLogger(RuntimeAuthenticationFailureHandler.class);
	...
	public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response,
		...
		DefaultRuntimeException ex = new DefaultRuntimeException(HttpStatus.UNAUTHORIZED, authenticationException);
		logger.error("{}", ex.getErrorCode(), ex);
		
		response.getOutputStream()
				.println(objectMapper.writeValueAsString(new RuntimeResponse<>().setType(ResponseType.ERROR)
						.setError(ex)));
	}
